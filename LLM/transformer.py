...
class MultiHeadAttentionLayer(torch.nn.Module):
    ...
    def _split_m_head(self, x):
        ...
        Args:
            x: Tensor, shape=(bs, seq_len, dim)
        
        Returnd:
            output: Tensor, shape=(bs, m_head, seq_len, dim // m_head)
        """
        b, s, d = x.shape
        x = x.view(b, s, self.m_head, -1)
        return x.permute(0, 2, 1, 3)
    
    def scaled_dot_production_attention(self, q, k, v, attention_mask, head_mask):
        """
        进行attention计算
        
        Args:
            q k v: Tensor, shape=(bs, m_head, seq_len, dim // m_head)
        ...
        if attention_mask is not None:
            # 想要v中某一个元素的weight趋近于0，则给对应weight加上一个负的极大值后，再下一步进行softmax后就会变成趋近于0
        ...
        batch, head, seqlen, dim = out.shape
        return out.transpose(1, 2).contiguous().view(batch, seqlen, head * dim)
    ...
    def forward(self, hidden_states,
        ...
        ############### 获取weight ###############
        return self.scaled_dot_production_attention(q, k, v, attention_mask, head_mask)
